...
async def add_target(
    webhook: str, name: str, chat_id: int, key: str | None, prefix: str
) -> bool:
    async with async_session() as session:
        async with session.begin():
            db_target = await session.scalar(
...
async def update_target(
    webhook: str, chat_id: int, update_data: dict[str, str]
) -> None:
    async with async_session() as session:
        async with session.begin():
            db_target = await session.scalar(select(Targets).where(Targets.id == id))
            if not db_target:
                return False

            await session.execute(
                update(Targets).where(Targets.id == id).values(update_data)
            )
            return True
